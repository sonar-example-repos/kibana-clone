...
The 8.19.10 release includes the following enhancements and fixes.

[float]
[[enhancement-v8.19.10]]
=== Enhancements
Elastic Security solution::
For the Elastic Security 8.19.10 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
...
The 8.19.9 release includes the following enhancements and fixes.

[float]
[[enhancement-v8.19.9]]
=== Enhancements
Data ingestion and Fleet::
* Adds the background task `FleetPolicyRevisionsCleanupTask` which removes excess policy revisions from the `.fleet-policies` index ({kibana-pull}242612[#242612]).

Kibana platform::
* The **API keys** management page now defaults to showing personal API keys only ({kibana-pull}245261[#245261]).
* Adds a warning when deleting API keys currently used by alerting rules ({kibana-pull}243353[#243353]).

[float]
[[fixes-v8.19.9]]
=== Fixes
Alerting::
* Fixes an issue that caused the Security alerts table to not update columns correctly when switching view modes ({kibana-pull}245253[#245253]).
* Adds `consecutiveMatches` to action context ({kibana-pull}244997[#244997]).
Elastic Observability solution::
* Handles alias resolution when checking lock index mappings ({kibana-pull}244559[#244559]).
Elastic Security solution::
For the Elastic Security 8.19.9 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes "now" and mixed-format date handling in the **Share** menu ({kibana-pull}245539[#245539]).
* Fixes an issue with authentication when multiple OIDC providers are configured ({kibana-pull}243869[#243869]).
* Fixes an ECS incompatibility with the `kibana_started.elasticsearch.waitTime` value in logs ({kibana-pull}245706[#245706]).
* Fixes the serialization of `meta.error` in JSON layouts. If it is an `Error` instance, only `message`, `name`, and `stack` are included. Other fields are no longer returned in the logs ({kibana-pull}244364[#244364]).
* Fixes an issue where Upgrade Assistant gave incorrect warnings about nodes exceeding the `storage low watermark` when the `max headroom` setting was configured ({kibana-pull}243906[#243906]).
...
[[release-notes-8.19.8]]
== {kib} 8.19.8
The 8.19.8 release includes the following enhancements and fixes.

[float]
[[enhancement-v8.19.8]]
=== Enhancements
Observability::
* Adds new sub-feature privileges for Synthetics global parameters  ({kibana-pull}243821[#243821]).

[float]
[[fixes-v8.19.8]]
=== Fixes
Alerting::
* Fixes pagination for the **Alerts** table, which was getting stuck on rule details pages ({kibana-pull}242275[#242275]).
* Fetches tracked alerts without depending on the task state ({kibana-pull}235253[#235253]).
Dashboards and Visualizations::
* Fixes an error that occurs when you deselect the **(blank)** option from the filter controls menu ({kibana-pull}242036[#242036]).
Data ingestion and Fleet::
* Adds retry behavior for `/api/fleet/agents` when transient issues with {{es}} are encountered ({kibana-pull}243105[#243105]).
* Fixes `ignore_above` mapping for `flattened` fields ({kibana-pull}238890[#238890]).
Discover::
* Fixes truncation for longer text in the Discover table ({kibana-pull}241440[#241440]).
Elastic Observability solution::
* Fixes AI assistant only appearing in the Log overview for documents with the `message` field ({kibana-pull}243437[#243437]).
* Improves handling of missing `error.id` in APM errors ({kibana-pull}243638[#243638]).
* Reverts showing transform errors across all SLO pages ({kibana-pull}243013[#243013]).
* Fixes the Kubernetes pod query by changing the gap policy to include zeros ({kibana-pull}239596[#239596]).
Elastic Security solution::
For the Elastic Security 8.19.8 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes the production build favicon's CSS specificity ({kibana-pull}243351[#243351]).
* Fixes an issue on the **Index management** page where screen reader text was different from visible text ({kibana-pull}243802[#243802]).
* Fixes an issue with keyboard focus getting trapped on pages using document preview ({kibana-pull}243791[#243791]).
Machine Learning::
* Preserves cell selection in Anomaly Explorer when you click out of the **Alerts** popover ({kibana-pull}244183[#244183]).
* Ensures deleted text in AI connector or Inference endpoint forms is not sent as empty strings ({kibana-pull}244059[#244059]).
* Fixes import and improves validation for Anomaly Detection and Data Frame Analytics jobs ({kibana-pull}242263[#242263]).
...
=== Enhancements
Machine Learning::
* Adds a feedback button to Anomaly Explorer and Single Metric Viewer ({kibana-pull}239883[#239883]).

[float]
[[fixes-v8.19.7]]
=== Fixes
Alerting::
* In Cases, tables no longer flash when the page is loading ({kibana-pull}240155[#240155]).
* Fixes an infinite loop issue occurring when the Investigation guide of a rule's details is too long ({kibana-pull}240472[#240472]).
Dashboards and Visualizations::
* Labels in Maps' **Create index** flow now render with the default **Use vector tiles** scaling as soon as label styling is applied (or after saving), without requiring a scaling toggle ({kibana-pull}240728[#240728]).
* Fixes an issue occurring when resetting a dashboard with unsaved changes after enabling `timeRestore` and setting a time range ({kibana-pull}239992[#239992]).
Data ingestion and Fleet::
* Allows Fleet setup retries on start in all environments ({kibana-pull}240342[#240342]).
Kibana platform::
* Ensures that the `max_tokens` parameter is correctly passed as expected when connecting to Anthropic ({kibana-pull}241212[#241212]) & ({kibana-pull}241188[#241188]).
* Removes the default fallback region for the Amazon Bedrock connector ({kibana-pull}241157[#241157]).
* Fixes index names causing incompatible cluster error when product docs are installed for multiple inference IDs ({kibana-pull}240506[#240506]).
Elastic Observability solution::
* Adds missing transaction action links ({kibana-pull}241336[#241336]).
* Fixes overlapping components in the Observability AI Assistant flyout on small screens ({kibana-pull}241026[#241026]).
* Excludes stale SLOs from "group by" stats ({kibana-pull}240077[#240077]).
* Prevents Kibana tool from failing when using a proxy ({kibana-pull}236653[#236653]).
Elastic Security solution::
For the Elastic Security 8.19.7 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Machine Learning::
* Ensures that the inference endpoints list loads properly even when using a custom inference provider ({kibana-pull}240189[#240189]).


[[release-notes-8.19.6]]
== {kib} 8.19.6

The 8.19.6 release includes the following known issues, enhancements, and fixes.
...
=== Enhancements
Elastic Security solution::
For the Elastic Security 8.19.6 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana security::
* Enhances error message by explaining why the role is considered malformed ({kibana-pull}239098[#239098]).

[float]
[[fixes-v8.19.6]]
=== Fixes
Alerting and cases::
* Updates `nodemailer` to `7.0.9` ({kibana-pull}238816[#238816]).
* Isolates the configuration parameters for the Tines connector to the server side ({kibana-pull}236863[#236863]).
Dashboards and Visualizations::
* Fixes bug that prevented you from deleting unsaved dashboard changes after you had enabled the `timeRestore` setting and selected a time and date using the date picker ({kibana-pull}239992[#239992]).
* Fixes error that occured when you selected a blank value in the options list ({kibana-pull}239791[#239791]).
* Fixes a bug in Lens that incorrectly assigned unsaved, ad-hoc dataview references ({kibana-pull}239431[#239431]).
* Fixes bug that broke the filter functionalty in table rows when any column is used as a formula in Lens ({kibana-pull}239222[#239222]).
* Fixes metric color assignment when breakdown and a max dimension are defined in Lens ({kibana-pull}238901[#238901]).
* Ensures adhoc dataviews from {esql} charts aren't being filtered out in the KQL search bar ({kibana-pull}238731[#238731]).
* Fixes bug where where an error in the visualization could cause the entire **Visualize Listing** page to error. This improves the error handling to make it easier to identify which visualization is causing problems ({kibana-pull}238355[#238355]).
* Fixes bug that prevented transforming of panels in sections from URL state ({kibana-pull}237382[#237382]).
Data ingestion and Fleet::
* Fixes bug that occured during an integration policy upgrade that prevented the new package global variables from being included and stale variable references from being removed ({kibana-pull}238542[#238542]).
* Fixes error when deleting orphaned integration policies and allows you to force delete orphaned integration policies using Fleet APIs ({kibana-pull}237875[#237875]).
* Fixes an issue where fields were resetting automatically when editing ingest pipeline settings ({kibana-pull}237509[#237509]).
Discover::
* Ensures that the combined filters are considered when selecting the correct fields for the query while the `courier:ignoreFilterIfFieldNotInIndex` advanced setting is enabled ({kibana-pull}238945[#238945]).
* Clears time field sorting after going from Classic to {esql} mode ({kibana-pull}235338[#235338]).
Elastic Observability solution::
* Removes unecessary `_source` from APM query ({kibana-pull}239205[#239205]).
* Uses word boundaries to only replace the metric name as an identifier and not as part of other text ({kibana-pull}238849[#238849]).
* Fixes error when Observability AI Assistant is disabled ({kibana-pull}238811[#238811]).
* Uses `runWithCache` for Fleet bulk operations ({kibana-pull}238326[#238326]).
* Disables max attempts for private locations sync task ({kibana-pull}237784[#237784]).
* Fixes recover alert while monitor is down ({kibana-pull}237479[#237479]).
* Fixes the AI Assistant button tooltip by closing the tooltip when you are not hovering over the button ({kibana-pull}237202[#237202]).
Kibana platform::
* Fixes files management flyout crashing ({kibana-pull}237588[#237588]).
Management::
* Fixes a bug where creating a new component template with `@custom` suffix in its name would update mappings of all unrelated data streams and a message would display asking to rollover conflicting ones ({kibana-pull}237952[#237952]).
Search::
* Fixes an issue in the RAG Playground where invalid fields displayed red styling but no error message. Error text is now shown to help users identify and correct form issues ({kibana-pull}238284[#238284]).
...
=== Fixes
Alerting::
* Rolls over the reporting data stream if its template version is newer than the version in the data stream's mappings ({kibana-pull}234119[#234119]).
Dashboards and Visualizations::
...
* Fixes malformed synthetics package policies ({kibana-pull}236176[#236176]).
Discover::
* Clears sort by `timestamp` when navigating from classic to ES|QL mode ({kibana-pull}235338[#235338]).
* Fixes sizing issues with the **Actions** column header ({kibana-pull}235227[#235227]).
Elastic Observability solution::
* Fixes the AI Assistant button's tooltip by closing it when you click anywhere else on the page ({kibana-pull}237202[#237202]).
...
=== Enhancements
Stack management::
* Adds IPv6 support to remote clusters address fields ({kibana-pull}233415[#233415]).

[float]
[[fixes-v8.19.4]]
=== Fixes
Dashboards and Visualizations::
* Fixes an issue with the Save modal that allowed duplicate saves of dashboards, visualizations, and other objects when clicking the **Save** button multiple times ({kibana-pull}233933[#233933]).
Elastic Security solution::
For the Elastic Security 8.19.4 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana security::
* Fixes an issue causing the API Keys Management page to break when trying to load API keys with null names ({kibana-pull}234083[#234083]).
Search solution::
* Fixes the Docs Explorer paging information to correctly render bold text instead of escaped HTML ({kibana-pull}234079[#234079]).
Observability solution::
* Fixes an issue in synthetics where multiple package policies are being created if user have monitors in multiple spaces ({kibana-pull}235021[#235021]).
...
=== Fixes
Alerting::
* Fixes an issue that prevents reports from showing in the Reporting UI when they're created in a non-default {kib} space ({kibana-pull}230876[#230876]).
Canvas::
* Fixes a rendering issue that affected progress elements in Canvas ({kibana-pull}232432[#232432]).
Dashboards and Visualizations::
* Ensures passing of the `allowLeadingWildcards` advanced setting for ES|QL charts in Lens ({kibana-pull}232609[#232609]).
* Removes the list of `availableOptions` from the ES|QL `VALUES_FROM_QUERY` saved object ({kibana-pull}231690[#231690]).
* Fixes older color mapping configuration in Lens ({kibana-pull}231563[#231563]).
* Fixes lost references when returning to unsaved dashboards with reference panels ({kibana-pull}231517[#231517]).
* Fixes screen reader-only header in the Dashboards list ({kibana-pull}230470[#230470]).
Discover::
* Disables sorting for JSON-like fields in ES|QL mode ({kibana-pull}231289[#231289]).
Elastic Security solution::
For the Elastic Security 8.19.3 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes an issue where the `unusedUrlsCLeanupTask` run interval did not update correctly when changed ({kibana-pull}231883[#231883]).
...
=== Enhancements
Search::
* Bumps default Gemini model for the Gemini Connector in Playground from Gemini 1.5 Pro to Gemini 2.5 Pro ({kibana-pull}230457[#230457]).
...
* Fixes screen reader-only header in the Dashboards list ({kibana-pull}230470[#230470]).
Elastic Observability solution::
* Fixes an issue with `setupLockManagerIndex` where if the first call failed, all subsequent calls would fail as well ({kibana-pull}230519[#230519]).
* Adds loading state to the **Install** and **Uninstall** buttons for Elastic documentation on the AI Assistant **Settings** page ({kibana-pull}229579[#229579]).

[[release-notes-8.19.1]]
== {kib} 8.19.1

The 8.19.1 release includes the following known issues and fixes.
...
=== Fixes
Dashboards and Visualizations::
* Fixes incorrect panel rendering when the **Defer loading panels below "the fold"** advanced setting is on ({kibana-pull}229662[#229662]).
* Fixes an issue in reports where a PNG or PDF export would offset and clip part of the visualization in *Lens* ({kibana-pull}228603[#228603]).
* Fixes ES|QL loading button state for long running queries in *Lens* ({kibana-pull}226565[#226565]).
* The `system` color mode now correctly applies the dark theme to charts and their legends when the system is in dark mode ({kibana-pull}218417[#218417]).
* Fixes a console error occurring when adding a Region map visualization to a dashboard ({kibana-pull}228669[#228669]).

Discover::
* Improves performance of breakdown field search ({kibana-pull}229335[#229335]).
* Fixes the field being picked for fetching stats in the sidebar popover ({kibana-pull}228969[#228969]).
* Fixes an issue where HTML search highlighting markup is incorrectly carried over into filter values when using the **Filter for value** field action in the **Log overview** tab ({kibana-pull}227652[#227652]).
* Fixes an issue where some Discover **Copy to clipboard** actions only worked when accessing Kibana in a secure browser context ({kibana-pull}229531[#229531]).

Elastic Observability solution::
* AI Assistant now works offline, allowing to use the knowledge base UI for cases like air-gapped environments ({kibana-pull}229874[#229874]).
* Fixes global parameters sync for non-default private locations ({kibana-pull}230157[#230157]).
* Fixes an issue with column sorting in the **Services** Errors table ({kibana-pull}229199[#229199]).
* Fixes an error in Uptime occurring when filters are missing in the parameters of a rule created using the API ({kibana-pull}229422[#229422]).

Elastic Security solution::
For the Elastic Security 8.19.1 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].

Kibana platform::
* Fixes a display issue with counter indicators on toast messages ({kibana-pull}229300[#229300]).
...
*Resolved* +
This issue is resolved in 8.19.3, 9.1.3, and 9.2.0.
====
...
Before you upgrade to 8.19.0, review the breaking changes, then mitigate the impact to your application.

[discrete]
.Change to variable syntax in {esql} queries
[%collapsible]
====
*Details* +
...
[[features-8.19.0]]
=== Features
{kib} 8.19.0 adds the following new and notable features.

Alerting::
* Adds the ability to schedule reports on a recurring basis and view previously scheduled reports ({kibana-pull}224849[#224849], {kibana-pull}221028[#221028]).
* Adds the `xpack.actions.email.services.enabled` {kib} setting, which allows you to enable or disable email services for email connectors ({kibana-pull}223363[#223363]).
* Adds the `xpack.actions.webhook.ssl.pfx.enabled` {kib} setting, which allows you to disable Webhook connector PFX file support for SSL client authentication ({kibana-pull}222507[#222507]).
...
Dashboards and Visualizations::
* Adds the **Create alert rule** action to ES|QL dashboard panels, usable from the panel context menu or by right-clicking a data point on the visualization. This rule allows you to generate an alert when the data on the chart crosses a certain threshold ({kibana-pull}217719[#217719]).
* Adds collapsible sections to dashboards that allow you to group panels into logical groups. Collapsible sections also help dashboards load faster by only loading their content when expanded ({kibana-pull}220877[#220877]).
* Adds the ability to mark a dashboard as favorite from within the dashboard in addition to the **Dashboards** page ({kibana-pull}201596[#201596]).
* Adds the ability to resize and move dashboard panels using a keyboard ({kibana-pull}208286[#208286]).
* Adds the ability to create {esql} controls from the dashboard **Controls** menu ({kibana-pull}219495[#219495]).
* Adds the ability to create {esql} controls by typing a question mark (`?`) when editing an {esql} visualization's query ({kibana-pull}216839[#216839]).
* Adds a **Compare to** badge for Metric charts in **Lens** ({kibana-pull}214811[#214811]).
Data ingestion and Fleet::
* Enables the automatic agent upgrades feature in {fleet} ({kibana-pull}219932[#219932]).
* Adds support for Cloud Connectors in CSPM ({kibana-pull}212200[#212200]).
Discover::
* Adds an **Attributes** tab to the document viewer when exploring OTel documents in the Observability solution view ({kibana-pull}222391[#222391]).
* Shows any available results when an ES|QL query times out as a result of the `search:timeout` advanced setting ({kibana-pull}219027[#219027]).
* Adds click actions for Stacktrace and Degraded Fields when exploring logs in Discover ({kibana-pull}214413[#214413]).
* Shows a **Load more** option instead of pagination when exploring Logs in Discover ({kibana-pull}211176[#211176]).
* Adds **Read more** and **Read less** functionality to Document view fields in Discover ({kibana-pull}215326[#215326]).
Elastic Observability solution::
...
Elastic Security solution::
For the Elastic Security 8.19.0 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].

Management::
* Adds autocomplete suggestions for {esql} queries in Console ({kibana-pull}219980[#219980]).
Search::
* Adds a section that enables adding, updating, and deleting query rules written on top of the Query Rules APIs to pin or exclude documents according to criteria you set ({kibana-pull}227226[#227226]).

For more information about the features introduced in 8.19.0, refer to <<whats-new,What's new in 8.19>>.

[[enhancements-and-bug-fixes-v8.19.0]]
...
==== Enhancements
Alerting::
* Ensures the **Reporting** page only shows reports generated in the current space ({kibana-pull}221375[#221375]).
* Moves the rule settings to a flyout ({kibana-pull}216162[#216162]).
...
* Adds a setting to control the data table density in **Lens** ({kibana-pull}220252[#220252]).
* When clicking "Open in Lens" from a visualization, **Lens** now opens in the same tab ({kibana-pull}217528[#217528]).
* Adds the ability to open links from **Vega** visualizations in a new tab by specifying the `"target": "_blank"` option for the `usermeta.embedOptions.loader` property of the Vega chart configuration ({kibana-pull}216200[#216200]).
* Adds globe projection improvements to **Maps** ({kibana-pull}212437[#212437]).
* Keeps the chart configuration when possible after editing the {esql} visualization's query ({kibana-pull}210780[#210780]).
* Allows the creation of dynamic aggregations controls for {esql} visualizations ({kibana-pull}210170[#210170]).
Data ingestion and Fleet::
...
* Adds a warning and a tooltip for explaining the `_score` column in **Discover** ({kibana-pull}211013[#211013]).
* Adds support for `command`/`ctrl` + click to open new Discover sessions in a separate tab, for example allowing to conduct simultaneous searches in parallel more efficiently ({kibana-pull}210982[#210982]).
* Updates styles for Color formatter to look like badges ({kibana-pull}189391[#189391]).
{esql} editor::
* The {esql} `LOOKUP_JOIN` command is now GA ({kibana-pull}225117[#225117]).
* The {esql} `COMPLETION` command is now available in technical preview ({kibana-pull}224811[#224811]).
* The {esql} `FORK` command is now available in technical preview ({kibana-pull}224680[#224680]).
* Adds suggestions for all operators when writing queries ({kibana-pull}223503[#223503]).
* Improves handling of long fields ({kibana-pull}223222[#223222]).
* Shows list of keyboard shortcuts at the bottom of the {esql} editor ({kibana-pull}221331[#221331]).
* Adds suggestions for full text search when writing queries ({kibana-pull}221239[#221239]).
* Adds full text search suggestions to `STATS ... WHERE` queries ({kibana-pull}220691[#220691]).
* Adds autocomplete suggestions for `STATS...WHERE` ({kibana-pull}216379[#216379]).
* Enables suggestions for the `CHANGE_POINT` command ({kibana-pull}218100[#218100]).
* Adds validation and autocomplete support for the `CHANGE_POINT` command ({kibana-pull}216043[#216043]).
* Adds highlighting to code examples in the in-product documentation ({kibana-pull}214915[#214915]).
* Suggests triple quotes when using `KQL` and `QSTR` functions ({kibana-pull}211457[#211457]).
Elastic Observability solution::
...
For the Elastic Security 8.19.0 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Adds an unused "URL" saved objects cleanup task to reduce upgrade downtime ({kibana-pull}220138[#220138]).
* Adds the ability to add badges to navigation subitems ({kibana-pull}214854[#214854]).
* Adds a badge to navigation items except section header ({kibana-pull}217301[#217301]).
* Adds back external link indicator to the side navigation ({kibana-pull}215946[#215946]).
* Kibana logging's pattern layout, used by default for the console appender, will now use a new default pattern layout: `[%date][%level][%logger] %message %error`. This includes the error name and stack trace if these were included in the log entry. To opt out of this behavior, you can omit the `%error` placeholder from your log pattern configuration in `kibana.yml` ({kibana-pull}219940[#219940]).
Kibana security::
* Adds a `defaultSolution` setting to spaces configuration to allow starting Kibana with its default space set to specific solution view ({kibana-pull}218360[#218360]).
* Adds an `object_src` directive to `Content-Security-Policy-Report-Only` header ({kibana-pull}209306[#209306]).
* Improves handling for operator-defined role mappings ({kibana-pull}208710[#208710]).
* Updates `js-yaml` to `4.1.0` ({kibana-pull}190678[#190678]).
Machine Learning::
* Updates NL-2-ESQL docs ({kibana-pull}224868[#224868]).
* Hides Adaptive Allocations Toggle in Serverless ({kibana-pull}224097[#224097]).
* Adds rare scripts job to preconfigured Security:Windows anomaly detection jobs ({kibana-pull}223041[#223041]).
* Adds new subAction for converse and converseStream for Bedrock ({kibana-pull}223033[#223033]).
* Adds a recommended query for the `CATEGORIZE` function in {esql} ({kibana-pull}222871[#222871]).
* Improves error messages in Kibana's AI Connector creation UI ({kibana-pull}221859[#221859]).
* Adds the ability to disable the AIOps features in {kib} ({kibana-pull}221286[#221286]).
* Prevents the download of models already present in other spaces and shows a warning ({kibana-pull}220238[#220238]).
* Enhances `No Results` state for Change Point Detection ({kibana-pull}219072[#219072]).
* Fixes some OpenAI models not accepting temperature for Inference service ({kibana-pull}218887[#218887]).
* Adds Voyage AI and DeepSeek icons ({kibana-pull}216651[#216651]).
* Enhances the display for anomaly time function values in anomaly tables ({kibana-pull}216142[#216142]).
* Improves the UX for deploying trained models ({kibana-pull}205699[#205699]).
Management::
* Adds a loader indicator when datasources are being loaded ({kibana-pull}225005[#225005]).
Sharing::
* Adds the ability to switch between relative and absolute time range when sharing objects ({kibana-pull}218056[#218056]).

[float]
[[fixes-v8.19.0]]
==== Fixes
Alerting::
* Reports are timing out and failing with an invalid header error ({kibana-pull}225919[#225919]).
* Fixes ignored dynamic templates ({kibana-pull}219875[#219875]).
* Updates task state version after execution ({kibana-pull}215559[#215559]).
* Fixes a regression that caused the cases actions to disappear from the Alerts table **Bulk actions** menu ({kibana-pull}215111[#215111]).
Dashboards and Visualizations::
* Forwards the secondary prefix correctly when the state value is an empty string (`None` option) in **Lens** ({kibana-pull}228183[#228183]).
* Fixes an issue where a **Lens** Partition chart (i.e. `Pie`) prevented the user from selecting a legacy palette ({kibana-pull}228051[#228051]).
* Fixes an accessibility issue where dashboard controls should be grouped as a labelled list ({kibana-pull}227633[#227633]).
* Fixes incorrectly disabled range slider tooltip and moves the **Delete control** button to be more visible ({kibana-pull}227295[#227295]).
* Fixes secondary metric styles to prevent wrapping in **Lens** ({kibana-pull}227234[#227234]).
* Keeps the **Save** button enabled in case of save error ({kibana-pull}227091[#227091]).
* Fixes an issue with dashboards not saving due to exceptionally high number of references in the request payload ({kibana-pull}225908[#225908]).
* Prevents Dashboard from recommending adhoc data views ({kibana-pull}225705[#225705]).
* Prevents Lens Embeddable `defaultTitle` from being overwritten with a custom title after reload ({kibana-pull}225664[#225664]).
* Fixes panel title synchronization with corresponding saved object when using `defaultTitle` ({kibana-pull}225237[#225237]).
* Refreshes "Values from a query" options for {esql} controls on dashboard reload ({kibana-pull}225101[#225101]).
* Fixes an issue with calculating the query for retrieving {esql} control values ({kibana-pull}214905[#214905]).
* Fixes an issue with the {esql} **Create control** suggestions not triggering if the query already contained a control ({kibana-pull}214833[#214833]).
* Fixes the visibility of the date picker when writing {esql} visualization queries ({kibana-pull}214728[#214728]).
* Fixes a performance issue with {esql} visualizations in case of errors in the query ({kibana-pull}225067[#225067]).
* Fixes dashboard control value changes causing multiple fetches ({kibana-pull}224761[#224761]).
* Prevents Lens from breaking when the table has no data [#217937]({kib-pull}217937]).
* Fixes an issue in **Lens** where reordering the groups within a layer would incorrectly assign the color mapping to a group other than the first ({kibana-pull}215426[#215426]).
* Fixes invalid dashboard displayed as 404 instead of showing validation error ({kibana-pull}211661[#211661]).
* Fixes an issue where custom ranges and multi-field values were not correctly colored based on selected color mapping configurations ({kibana-pull}207957[#207957]).
* Adds `useCustomDragHandle` property ({kibana-pull}210463[#210463]).
* Fixes presentation panel styles ({kibana-pull}210113[#210113]).
* Fixes an issue in *Lens* tables where a split-by metric on a term rendered incorrect colors in table cells ({kibana-pull}208623[#208623]).
* Fixes an issue with assigning color mappings when multiple layers are defined in *Lens* ({kibana-pull}208571[#208571]).
Data ingestion and Fleet::
* Fixes backfill logic for agentless support across all packages ({kibana-pull}225767[#225767]).
* Fixes a UI error that occurred when no tag filter was selected ({kibana-pull}225413[#225413]).
* Reverts "Added instructions for installing {agent} complete" ({kibana-pull}223520[#223520]).
* Fixes styled component theme lookup issue ({kibana-pull}221979[#221979]).

Discover::
* Adds singulars/plurals to expiration badges ({kibana-pull}227035[#227035]).
* Fixes invalid input highlight in **Data View** flyout ({kibana-pull}226822[#226822]).
* Fixes an issue causing **Discover** to freeze when dragging & dropping columns with animations disabled ({kibana-pull}226592[#226592]).
* Fixes an issue causing an error when updating then deleting a saved query ({kibana-pull}226569[#226569]).
* Fixes an issue causing the selected document to change when resizing flyout with keyboard ({kibana-pull}225594[#225594]).
* Fixes an issue where an {esql} query was overwritten when edited while the previous request was still running ({kibana-pull}224671[#224671]).
* The request URL now appears correctly again when inspecting an ES|QL request ({kibana-pull}221816[#221816]).
* Fixes an issue causing the icon of field list to change when mapping changes from unmapped to mapped ({kibana-pull}221308[#221308]).
* Fixes the parsing of index patterns in the **Inspect** feature of Kibana. Previously, certain index pattern strings were not being parsed and displayed correctly in the **Inspect** feature ({kibana-pull}221084[#221084]).
* Fixes an issue that causes redirects from the deprecated **Logs Stream** and **Logs Explorer** routes to **Discover** to lose some context such as the selected time range or KQL query ({kibana-pull}215867[#215867]).
* Excludes only {es} metadata fields from the Summary column instead of all fields starting with `_` ({kibana-pull}213255[#213255]).
* Fixes inability to clear Document ID in data view field editor preview ({kibana-pull}220891[#220891]).
* Fixed multiple accessibility issues, including adding missing aria labels and column headers, improving keyboard navigation and interactions, and improving focus changes when interacting with **Discover** features https://github.com/elastic/kibana/issues?q=state:closed%20label:Project:Accessibility%20label:v8.19.0%20label:Team:DataDiscovery[View list of fixes].
{esql} editor::
* Fixes wrong validation on expressions between aggregations ({kibana-pull}227989[#227989]).
* Hides lookup index hidden indices from autocomplete ({kibana-pull}227819[#227819]).
* Fixes several issues with `BUCKET` function signatures ({kibana-pull}222553[#222553]).
* Fixes validation issues with the `COALESCE` function ({kibana-pull}222425[#222425]).
* Fixes incorrect suggestions after a variable such as `?value` when using the `WHERE` command ({kibana-pull}222312[#222312]).
* Fixes an issue with suggestions after using triple quotes ({kibana-pull}221200[#221200]).
* Fixes a validation issue when using asterisks `*` in queries ({kibana-pull}219832[#219832]).
* Fixes an issue with incorrect source validation in case of unknown patterns ({kibana-pull}218352[#218352]).
* Fixes a display issue with the editor's menus on Safari ({kibana-pull}218167[#218167]).
* Fixes a display issue with descriptions in suggestion tooltips ({kibana-pull}218067[#218067]).
* Correctly enables the time picker when time parameters are used with cast ({kibana-pull}215820[#215820]).
* Fixes an issue preventing warnings to display correctly when they include escaped quotes ({kibana-pull}213685[#213685]).
* Fixes a validation issue when a named parameter is used as a function ({kibana-pull}213355[#213355]).
* Fixes an issue with suggestions for the `WHERE` command in case of a multiline query ({kibana-pull}213240[#213240]).
...
For the Elastic Security 8.19.0 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes "How's the navigation working for you?" shown with each session ({kibana-pull}227199[#227199]).
* Hides Kibana's header and side navigation when trying to print or use the dashboard export with print mode ({kibana-pull}227095[#227095]).
* Removes listing limit warning ({kibana-pull}217945[#217945]).
* Fixes image resize for images used in `xpack.security.loginAssistanceMessage` ({kibana-pull}212035[#212035]).
* Passes the correct namespace to `migrateInputDocument` ({kibana-pull}222313[#222313]).
* Fixes an issue preventing solution navigation submenu items from being displayed when the navigation is collapsed ({kibana-pull}227705[#227705]).
Machine Learning::
* Fixes handling of time ranges in embedded anomaly swim lanes ({kibana-pull}225803[#225803]).
* Fall backs to the default value when `lookbackInterval` is empty in Anomaly Detection rules ({kibana-pull}225249[#225249]).
* Fixes unknown fields not supported in Data Visualizer and Field Statistics ({kibana-pull}223903[#223903]).
* Fixes 'Use full data' button issue which could cause page to crash ({kibana-pull}217291[#217291]).
* Fixes permission to view ML nodes ({kibana-pull}215503[#215503]).
* Fixes chart in single metric anomaly detection wizard ({kibana-pull}214837[#214837]).
* Fixes incorrect overall swim lane height in Anomaly Explorer ({kibana-pull}213245[#213245]).
* Prevents multiple clicks in the Delete Model dialog ({kibana-pull}211580[#211580]).
* Fixes further deployment of models after a failed deployment ({kibana-pull}211459[#211459]).
* AIOps Hides saved query controls ({kibana-pull}210556[#210556]).
Management::
* Fixes spaces search functionality for spaces created with avatar type as image ({kibana-pull}220398[#220398]).
* Fixes flyout styling issues in data views **Edit** flyout ({kibana-pull}228078[#228078]).
...
The 8.18.7 release includes the following fixes.

[float]
[[fixes-v8.18.7]]
=== Fixes
Elastic Security solution::
For the Elastic Security 8.18.7 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana security::
* Fixes an issue causing the API Keys Management page to break when trying to load API keys with null names ({kibana-pull}234083[#234083]).
Search solution::
* Fixes the Docs Explorer paging information to correctly render bold text instead of escaped HTML ({kibana-pull}234079[#234079]).
...
The 8.18.5 release includes the following fixes.

[float]
[[fixes-v8.18.5]]
=== Fixes
Dashboards and Visualizations::
* Fixes an issue with the plus sign `+` not being properly encoded when it's part of a date math expression ({kibana-pull}230469[#230469]).
* Fixes an issue in Firefox preventing you from scrolling in the Lens editor flyout ({kibana-pull}228625[#228625]).
* Fixes an issue preventing you from saving linked TSVB visualizations when changing data views ({kibana-pull}228685[#228685]).
* Fixes a console error occurring when adding a Region map visualization to a dashboard ({kibana-pull}228669[#228669]).
Data ingestion and Fleet::
* Fixes an issue that could cause some SSL configurations set using the UI to be overridden ({kibana-pull}230758[#230758]).
* Fixes an issue where the SSL configuration isn't properly added to agent policies with pre-configured output types ({kibana-pull}230211[#230211]).
* Fixes agentless integrations using 'organization', 'division', or 'team' data fields being overwritten by package agentless metadata on the agent policy ({kibana-pull}230479[#230479]).
Discover::
* Improves performance of breakdown field search ({kibana-pull}229335[#229335]).
* Enables the **Save query** button after making changes ({kibana-pull}229053[#229053]).
* Fixes the field being picked for fetching stats in the sidebar popover ({kibana-pull}228969[#228969]).
* Fixes an issue where some Discover **Copy to clipboard** actions only worked when accessing Kibana in a secure browser context ({kibana-pull}229531[#229531]).
Elastic Observability solution::
* Prevents from performing destructive actions using Elastic AI Assistant ({kibana-pull}229497[#229497]).
* Fixes an issue with column sorting in the **Services** Errors table ({kibana-pull}229199[#229199]).
* Fixes an issue with dependencies infinitely loading when no data exists ({kibana-pull}228094[#228094]).
* Fixes an error in Uptime occurring when filters are missing in the parameters of a rule created using the API ({kibana-pull}229422[#229422]).
Elastic Security solution::
For the Elastic Security 8.18.5 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes an issue with overlapping action icons in the **Data views** menu when on mobile ({kibana-pull}229771[#229771]).
* Fixes a display issue with counter indicators on toast messages ({kibana-pull}229300[#229300]).
* Fixes an issue leading to some features not being visible when updating a space solution view to Classic ({kibana-pull}230385[#230385]).
Machine Learning::
* Removes `removeIfExists` from the synchronization task scheduler ({kibana-pull}228783[#228783]).
...
Elastic Observability solution::
* Improves system prompt and instructions to work better with Claude models ({kibana-pull}221965[#221965]).

Elastic Security solution::
For the Elastic Security 8.18.3 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].

Kibana security::
* The **Invalidate user sessions** API is now generally available ({kibana-pull}224076[#224076]).
...
=== Fixes
Alerting::
* Fixes an issue causing Kibana to keep restarting when the `cancelAlertsOnRuleTimeout` Kibana configuration setting was set to `false` ({kibana-pull}222263[#222263]).
* Resolves multiple issues in the Watcher UI that were introduced in 8.18.0 and 8.18.1. This includes the table not displaying more than 10 watches, an error banner appearing unexpectedly in certain scenarios, and the search bar functionality not working as expected ({kibana-pull}223898[#223898]).
...
* Fixes an issue that caused errors when rules ran during an active maintenance window that had filters and a matching rule category ({kibana-pull}221702[#221702]).
...
Data ingestion and Fleet::
* Fixes the `UnenrollInactiveAgentsTask` task to correctly only unenroll agents inactive for longer than the value defined by the `unenroll_timeout` setting ({kibana-pull}222592[#222592]).
* Fixes an issue that could prevent some integrations from correctly showing as installed ({kibana-pull}221624[#221624]).

Discover::
* In version 8.18.0, we changed the way time ranges are handled for Discover ES|QL CSV exports ({kibana-pull}216792[#216792]). We realized that this change caused an issue with CSV Post URLs now using an absolute time range instead of a relative one, leading to duplicate reports being generated. We've reverted this change while we investigate different solutions ({kibana-pull}223249[#223249]).
...
* Fixes an issue that could cause losing unsaved changes after manually running a Synthetics test ({kibana-pull}222503[#222503]).
...
* Avoids adding tool instructions to the system messages when tools are disabled ({kibana-pull}223278[#223278]).
...
* Fixes an issue preventing metrics graphs from correctly showing in the Services **Inventory** page ({kibana-pull}222439[#222439]).
...
Machine Learning::
* Fixes an issue with the Bedrock connector not using the action proxy configuration ({kibana-pull}224130[#224130]).
...
* File upload: Fixes adding additional field when a pipeline is blank ({kibana-pull}222775[#222775]).
* Fixes the inference endpoint assignment to the trained model object ({kibana-pull}222076[#222076]).
* File upload: Correctly restricts auto-deployment to Elastic models ({kibana-pull}221357[#221357]).
...
// tag::known-issue-222127[]
.Upgrading Kibana from 8.x to 8.18.2 fails due to a configuration conflict in the kibana.yml file.
[%collapsible]
====
**This issue is resolved in version 8.18.3** ({kibana-pull}222263[#222263]).

*Details* +
Upgrading {kib} from version 8.x to 8.18.2 fails due to a configuration conflict if `xpack.alerting.cancelAlertsOnRuleTimeout` is set to `false` in the `kibana.yml` file. {kib} fails to boot and shows a fatal error message in the {kib} logs that's similar to the following:
...
. Remove the `xpack.alerting.cancelAlertsOnRuleTimeout: false` setting from the `kibana.yml` file.
. Restart {kib} to apply the changes.

====
// end::known-issue-222127[]
...
For more information, check https://github.com/elastic/docs-content/issues/2088[#2088]
====
// end::known-issue-2088[]
...
The 8.17.10 release includes the following fixes.
...
* Fixes a display issue with counter indicators on toast messages ({kibana-pull}229300[#229300]).
* Fixes an issue leading to some features not being visible when updating a space solution view to Classic ({kibana-pull}230385[#230385]).
* Decreases the loading time drastically when loading Index Management component templates ({kibana-pull}228709[#228709]).
...
The 8.17.8 release includes the following known issues and fixes.
...
The 8.17.7 release includes the following known issues, enhancements, and fixes.
...
The 8.17.6 release includes the following known issues, enhancements, and fixes.
...
The 8.17.5 release includes the following known issues, enhancements, and fixes.
...
The 8.17.4 release includes the following known issues and bug fixes.
...
[[release-notes-8.17.3]]
== {kib} 8.17.3
The 8.17.3 release includes the following known issues and bug fixes.
...
=== Bug fixes
Elastic Observability solution::
* Allows editing of non-resolving log views ({kibana-pull}210633[#210633]).
* Removes `error.id` in `getErrorGroupMainStatistics` queries ({kibana-pull}210613[#210613]).
Elastic Security solution::
For the Elastic Security 8.17.3 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana security::
* Fixes the generated name for imported package titles ({kibana-pull}210916[#210916]).
* Fixes package name validation on the Data streams page ({kibana-pull}210770[#210770]).
* Handles empty categorization results from LLMs ({kibana-pull}210420[#210420]).
Platform::
* Adds versioning to inventory_view_saved_object ({kibana-pull}207007[#207007]).
Sharing::
* Reinstates switch to support generating public URLs for Embed when supported ({kibana-pull}207383[#207383]).

[[release-notes-8.17.2]]
== {kib} 8.17.2
The 8.17.2 release includes the following known issues and bug fixes.
...
=== Bug fixes
Alerting::
* Updates handling of invalid RRule params and prevents infinite looping ({kibana-pull}205650[#205650]).
* Fixes an issue with saving ECS group by fields at the root level for query DSL rules ({kibana-pull}203769[#203769]).
* Fixes race condition in alerting rules data view selector ({kibana-pull}203654[#203654]).
Data ingestion and Fleet::
* Fixes an issue with showing all agent tags when agent list is filtered ({kibana-pull}205163[#205163]).
Elastic Observability solution::
...
Elastic Security solution::
For the Elastic Security 8.16.3 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana security::
...
====
*Details* +
The Uptime app is already hidden from Kibana when there is no recent Heartbeat data, but will be completely removed in a future version. You should migrate to Synthetics as an alternative. For more details, refer to the {observability-guide}/uptime-intro.html[Uptime documentation].
====
